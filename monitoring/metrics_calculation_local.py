...
def calculate_metrics_postgresql(
    curr, i, unseen_df, reference_data, file_path, target, quick_train, prediction
):
...
async def batch_monitoring_backfill():
    ...
    reference_data = apply_model_to_data(
        model, run_id, reference_data.drop(columns=[target, prediction])
    )
    ...
    with psycopg.connect(CONNECTION_STRING_DB, autocommit=True) as conn:
        ...
        for i in range(0, 5):
            ...
            unseen_df = apply_model_to_data(
                model, run_id, unseen_df.drop(columns=[target])
            )
